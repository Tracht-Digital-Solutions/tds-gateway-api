...
describe("identity", () => {
  ...
  it("composes standalone — it declares no dependsOn", () => {
    // TDShop composes into BOTH products — the panel (catalogue management)
    // and the customer portal (the placement widget). A dependency on another
    // extension would make it uninstallable in whichever product lacks that one.
    expect(manifest.dependsOn ?? []).toHaveLength(0);
  });
});

describe("permissions", () => {
  it("declares the four rights the backend module also declares", () => {
    // These must match `ShopModule::permissions()` exactly. The panel resolves
    // a grant against the API's catalogue, so a right declared on only one side
    // is either ungrantable or ungated — and both fail silently.
    expect(manifest.permissions?.map((p) => p.id)).toEqual([
      "shop:read",
      "shop:write",
      "shop:orders",
      "shop:sync",
  ...
  it("gates every route — a catalogue screen is staff-only", () => {
    for (const route of manifest.routes ?? []) {
      expect(route.permission, `route ${route.pattern} is ungated`).toBeTruthy();
      expect(route.permission, `route ${route.pattern}`).toMatch(/^shop:/);
    }
  });

  it("leaves exactly ONE widget ungated, and it is the customer-facing one", () => {
    // `shop-picks` is the product placement in the customer portal's
    // dashboard. It is shown TO customers, so a `shop:*` gate would hide it
    // from precisely the audience it exists for. Every other widget here is
    // staff-facing and must stay gated.
    //
    // Pinned as an exact set rather than "at least one", because the failure
    // this guards runs both ways: gating this widget makes the whole placement
    // feature invisible in the portal, and ungating another one leaks
    // catalogue counts to every logged-in customer.
    const ungated = (manifest.widgets ?? []).filter((w) => !w.permission).map((w) => w.id);
    expect(ungated).toEqual(["shop-picks"]);

    for (const widget of manifest.widgets ?? []) {
      if (widget.id === "shop-picks") continue;
      expect(widget.permission, `widget ${widget.id} is ungated`).toMatch(/^shop:/);
    }
  });
...
describe("nav, routes and widgets", () => {
  ...
  it("points every specifier at its OWN package", () => {
    ...
    for (const spec of specifiers) {
      expect(spec, `${spec} does not live in this package`).toMatch(
        /^@tracht-digital-solutions\/tds-ext-shop\//,
      );
    }
  });
  ...
  it("points the widget at the endpoint its island actually reads", () => {
    expect(manifest.widgets?.[0]?.dataEndpoint).toBe("/shop/summary");
  });

  it("namespaces its nav, widget and settings ids under the module", () => {
    // These share one namespace across every composed extension.
    for (const n of manifest.nav ?? []) expect(n.id).toMatch(/^shop/);
    for (const w of manifest.widgets ?? []) expect(w.id).toMatch(/^shop/);
    for (const s of manifest.settings ?? []) expect(s.id).toMatch(/^shop/);
  });
...
describe("i18n", () => {
  ...
  it("namespaces its i18n keys so they cannot collide with another extension", () => {
    for (const key of Object.keys(manifest.i18n?.de ?? {})) {
      expect(key, `${key} is not namespaced`).toMatch(/^shop\./);
    }
  });
});
...
describe("composition", () => {
  ...
  it("surfaces its route, nav entry and settings panel through the composition", () => {
    ...
    expect(composed.routes.map((r) => r.pattern)).toContain("/shop");
    expect(composed.nav.map((n) => n.id)).toContain("shop");
    expect(composed.settings.map((s) => s.id)).toContain("shop");
  });
});
