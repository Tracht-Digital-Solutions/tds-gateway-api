...
describe("identity", () => {
  it("declares the id the product composes it under", () => {
    expect(manifest.id).toBe("messages");
  });
  ...
  it("composes standalone — it declares no dependsOn", () => {
    // The thread owns its own table and reads auth from the core UserContext.
...
describe("permissions", () => {
  it("declares the read/write pair the backend gates on", () => {
    ...
    expect(ids).toContain("messages:read");
    expect(ids).toContain("messages:write");
  });
  ...
  it("gates the page and the widget on read, never on nothing", () => {
    ...
    for (const route of manifest.routes ?? []) {
      expect(route.permission, `route ${route.pattern} is ungated`).toBe("messages:read");
    }
    for (const widget of manifest.widgets ?? []) {
      expect(widget.permission, `widget ${widget.id} is ungated`).toBe("messages:read");
    }
  });
...
describe("nav, routes and widgets", () => {
  ...
  it("points every specifier at its OWN package", () => {
    ...
    for (const spec of specifiers) {
      expect(spec, `${spec} does not live in this package`).toMatch(
        /^@tracht-digital-solutions\/tds-ext-messages\//,
      );
    }
  });
  ...
  it("points the widget at the endpoint its island actually reads", () => {
    expect(manifest.widgets?.[0]?.dataEndpoint).toBe("/messages/summary");
  });

  it("namespaces its nav, widget and settings ids under the module", () => {
    // These share one namespace across every composed extension.
    for (const n of manifest.nav ?? []) expect(n.id).toMatch(/^messages/);
    for (const w of manifest.widgets ?? []) expect(w.id).toMatch(/^messages/);
    for (const s of manifest.settings ?? []) expect(s.id).toMatch(/^messages/);
  });
...
describe("i18n", () => {
  ...
  it("namespaces its i18n keys so they cannot collide with another extension", () => {
    for (const key of Object.keys(manifest.i18n?.de ?? {})) {
      expect(key, `${key} is not namespaced`).toMatch(/^messages\./);
    }
  });
});
...
describe("composition", () => {
  ...
  it("surfaces its route and nav entry through the composition", () => {
    ...
    expect(composed.routes.map((r) => r.pattern)).toContain("/messages");
    expect(composed.nav.map((n) => n.id)).toContain("messages");
  });
});
