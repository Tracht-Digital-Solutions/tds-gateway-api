...
describe("identity", () => {
  it("declares the id the product composes it under", () => {
    expect(manifest.id).toBe("live-chat-cta");
  });
...
describe("permissions", () => {
  it("declares the read/write pair the backend gates on", () => {
    ...
    expect(ids).toContain("live-chat:read");
    expect(ids).toContain("live-chat:write");
  });
  ...
  it("points every specifier at its OWN package", () => {
    ...
    for (const spec of specifiers) {
      expect(spec, `${spec} does not live in this package`).toMatch(
        /^@tracht-digital-solutions\/tds-ext-live-chat-cta\//,
      );
    }
  });
...
describe("composition", () => {
  ...
  it("surfaces its route, nav entry and settings panel through the composition", () => {
    ...
    // The nav entry is `live-chat`; the settings panel is `live-chat-cta`.
    expect(composed.routes.map((r) => r.pattern)).toContain("/live-chat");
    expect(composed.nav.map((n) => n.id)).toContain("live-chat");
    expect(composed.settings.map((s) => s.id)).toContain("live-chat-cta");
  });
});
