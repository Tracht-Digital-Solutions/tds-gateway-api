...
describe("identity", () => {
  it("declares the id the product composes it under", () => {
    expect(manifest.id).toBe("contact-tickets");
  });
...
describe("permissions", () => {
  it("declares the read/write pair the backend gates on", () => {
    ...
    expect(ids).toContain("contact:read");
    expect(ids).toContain("contact:write");
  });
  ...
  it("gates the page and the widget on read, never on nothing", () => {
    ...
    for (const route of manifest.routes ?? []) {
      expect(route.permission, `route ${route.pattern} is ungated`).toBe("contact:read");
    }
    for (const widget of manifest.widgets ?? []) {
      expect(widget.permission, `widget ${widget.id} is ungated`).toBe("contact:read");
    }
  });
...
describe("nav, routes and widgets", () => {
  ...
  it("points every specifier at its OWN package", () => {
    ...
    for (const spec of specifiers) {
      expect(spec, `${spec} does not live in this package`).toMatch(
        /^@tracht-digital-solutions\/tds-ext-contact-tickets\//,
      );
    }
  });
  ...
  it("points the widget at the endpoint its island actually reads", () => {
    expect(manifest.widgets?.[0]?.dataEndpoint).toBe("/contact/summary");
  });

  it("namespaces its nav, widget and settings ids under the module", () => {
    // These share one namespace across every composed extension.
    for (const n of manifest.nav ?? []) expect(n.id).toMatch(/^contact/);
    for (const w of manifest.widgets ?? []) expect(w.id).toMatch(/^contact/);
    for (const s of manifest.settings ?? []) expect(s.id).toMatch(/^contact/);
  });
...
describe("i18n", () => {
  ...
  it("namespaces its i18n keys so they cannot collide with another extension", () => {
    for (const key of Object.keys(manifest.i18n?.de ?? {})) {
      expect(key, `${key} is not namespaced`).toMatch(/^contact./);
    }
  });
});
