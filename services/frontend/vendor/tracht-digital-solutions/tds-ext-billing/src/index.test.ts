...
describe("identity", () => {
  it("declares the id the product composes it under", () => {
    expect(manifest.id).toBe("billing");
  });
  ...
  it("composes standalone — it declares no dependsOn", () => {
    // invoice.customer_id references the tds-ext-customers directory softly, so
    // a product may compose billing without it.
...
describe("permissions", () => {
  it("declares the read/write pair the backend gates on", () => {
    ...
    expect(ids).toContain("billing:read");
    expect(ids).toContain("billing:write");
  });
...
describe("nav, routes and widgets", () => {
  ...
  it("points every specifier at its OWN package", () => {
    ...
    for (const spec of specifiers) {
      expect(spec, `${spec} does not live in this package`).toMatch(
        /^@tracht-digital-solutions\/tds-ext-billing\//,
      );
    }
  });
  ...
  it("points the widget at the endpoint its island actually reads", () => {
    expect(manifest.widgets?.[0]?.dataEndpoint).toBe("/billing/summary");
  });

  it("namespaces its nav, widget and settings ids under the module", () => {
    // These share one namespace across every composed extension.
    for (const n of manifest.nav ?? []) expect(n.id).toMatch(/^billing/);
    for (const w of manifest.widgets ?? []) expect(w.id).toMatch(/^billing/);
    for (const s of manifest.settings ?? []) expect(s.id).toMatch(/^billing/);
  });
...
describe("i18n", () => {
  ...
  it("namespaces its i18n keys so they cannot collide with another extension", () => {
    for (const key of Object.keys(manifest.i18n?.de ?? {})) {
      expect(key, `${key} is not namespaced`).toMatch(/^billing\./);
    }
  });
});
...
describe("composition", () => {
  ...
  it("surfaces its route, nav entry and settings panel through the composition", () => {
    ...
    expect(composed.routes.map((r) => r.pattern)).toContain("/rechnungen");
    expect(composed.nav.map((n) => n.id)).toContain("billing");
    expect(composed.settings.map((s) => s.id)).toContain("billing");
  });
});
