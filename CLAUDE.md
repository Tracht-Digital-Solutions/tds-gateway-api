...
- **Both architectures depend on it.** Today it fronts the legacy services (`auth`,
  ...
  after the panel-platform cutover it must route to `tds-core-panel-api` + `tds-auth-api`.
...
See `MIGRATION-STATUS.md` at the root for the legacy→panel replacement map.
